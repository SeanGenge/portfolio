...
bool Direct3D::Initialise(int width, int height, HWND windowHandle, bool fullscreen, bool vsync)
{
	...
	D3D11_TEXTURE2D_DESC depthBufferDescription;				//This struct will describe our depth buffer texture
	D3D11_DEPTH_STENCIL_DESC depthStencilDescription;			//This struct will describe how the depth buffer will used
	D3D11_DEPTH_STENCIL_VIEW_DESC depthStencilViewDescription;	//We use this struct to set up options and settings for the depth buffer
	D3D11_RASTERIZER_DESC rasterDescription;					//This raster description lets us set some options about how the games turns our 3D into 2D
	D3D11_VIEWPORT viewport;									//This view port object allows us to define which part of our screen will be rendered into.
	...
	{
		//...and look for one with the same size as the window we are created
	...
	swapChainDescription.BufferCount = 2;									//We then set the number of buffers we want on our swap chain, 1 is enough for us
	...
	D3D_FEATURE_LEVEL featureLevel[] = { D3D_FEATURE_LEVEL_11_0, D3D_FEATURE_LEVEL_10_0 };	//We are going to target the DirectX 11 and DX10 feature levels
	...
	//All of this code regarding the depth buffer will be covered later, I think we've gone through enough for now :P
	...
	if(FAILED(m_device->CreateRasterizerState(&rasterDescription, &m_rasterState)))	//After we have the raster setting we use the device to create a raster state
	{
		return false;
	}
	...
	//A viewport is a section of the screen that graphics are rendered into. It is possible to have multiple view ports for split screen multiplayer game. We only wnat one though.
