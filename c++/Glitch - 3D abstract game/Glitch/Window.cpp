...
bool Window::Initialise()
{
	...
	m_windowHandle = CreateWindowEx(WS_EX_APPWINDOW,	//This is the extended window style, it allows us to define special functionality for the window, but we just want a simple app window
		...
		WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_POPUP,	//This is the window style, you can create interesting windows by ORing styles together
...
void Window::Start()
{
	...
	MSG msg;	//This struct stores the received message

	bool running = true;	//A flag used to keep the pump running, if this is set to false then the application will quit

	while (running)
	{
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE))	//If is a message, store it in msg and remove it from the message queue
		{
			TranslateMessage(&msg);		//Translate the message...
			DispatchMessage(&msg);		//...and pass it off to the Window Procedure we registered earlier
			if (msg.message == WM_QUIT)		//If we received a Quit message then terminate the message pump and quit the application
			{
				running = false;
			}
		}
		...
		{
			LARGE_INTEGER currentCount;					//Here we get the current count of the Performance Counter
			...
			m_lastCount = currentCount;	//We set the last count value to the current count so that next frame we still have the count from this frame
		}
	}
}
...
void Window::Shutdown()
{
	...
	DestroyWindow(m_windowHandle);		//We need to destory our window
	m_windowHandle = NULL;

	UnregisterClass(m_windowName, m_hInstance);	//An unregister our window class
	m_hInstance = NULL;
...
{
	//The message proc now updates the Input Controller when Input messages are received
	switch (message)
	{
	case WM_KEYDOWN:
