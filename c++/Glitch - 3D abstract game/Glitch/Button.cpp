...
Button::Button(int width, int height, Texture* texture, LPCWSTR text, Vector2 position, 
	SpriteBatch* spriteBatch, SpriteFont* font, InputController* input,
	std::function<void()> onClickFunction)
{
	...
	m_text = text;
	m_position = position;
	m_textColour = Color(1.0f, 1.0f, 1.0f, 1.0f);
...
void Button::Update()
{
	...
	if (m_input->GetMouseX() > (m_position.x - halfWidth) && m_input->GetMouseX() < (m_position.x + halfWidth) && m_input->GetMouseY() > (m_position.y - halfHeight) && m_input->GetMouseY() < (m_position.y + halfHeight))
	{
...
void Button::Render()
{
	...
	m_spriteBatch->Draw(m_buttonTexture->GetShaderResourceView(), m_position, &m_textureWindow, Color(1.0f, 1.0f, 1.0f, 1.0f), 0, m_origin);
