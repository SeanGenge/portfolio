...
Mesh::Mesh()
{
	...
	m_minVector = Vector3::Zero;
	m_maxVector = Vector3::Zero;
...
bool Mesh::Initialise(Direct3D* direct3D, Shader* meshShader)
{
	...
	m_centre = Vector3::Zero;
	m_radius = 1.0f;
	m_minVector = Vector3(-1.0f, -1.0f, 0);
	m_maxVector = Vector3(1.0f, 1.0f, 0);
...
bool Mesh::Initialise(Direct3D* direct3D, const char* filename, Color defaultColour, Texture* texture, Shader* meshShader)
{
	...
	m_radius = (m_minVector - m_maxVector).Length() / 2;
	m_centre = (m_minVector - m_maxVector) / 2;
